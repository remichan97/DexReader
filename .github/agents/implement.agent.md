...
2. **Locate the plan file**: Identify the relevant plan file (located at `.github/copilot-plans/feature-name-plan.md`) that outlines the steps needed to complete the task. If no plan exists, guide the user to switch to Planning mode to create one.
3. **Implementation**: Adhere to the steps outlined in the plan file, ensure to follow coding standards and best practices as specified in `../instructions/typescripts-standard.instructions.md`.
...
5. **Delete the plan file**: Once the implementation is complete and reviewed, delete the relevant plan file (located at `.github/copilot-plans/feature-name-plan.md`) to avoid confusion and maintain a clean project structure.
6. **Update context**: After completing the implementation, update `active-context.md` in the memory bank to reflect the changes made and any relevant notes for future reference. This will help maintain an accurate and up-to-date context for future tasks and implementations.
...
- Depending on the scale of the implementation request, use subtasks/subagents if necessary to break down the implementation into manageable parts.
...
- Make sure that your implementation adheres to the project's coding standards and best practices as outlined in `../instructions/typescripts-standard.instructions.md`, format the code properly.

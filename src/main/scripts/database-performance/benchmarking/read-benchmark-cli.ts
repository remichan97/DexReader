 ...
 * Results are automatically saved to benchmark-results/read-benchmarks.json
 *
 * Usage:
 *   npm run benchmark:db                    # Run benchmarks with defaults
 *   npm run benchmark:db -- --iterations 20 # More iterations for accuracy
 *   npm run benchmark:db -- --verbose       # Show detailed progress
 *
 * Options:
 ...
 *
 * Output: benchmark-results/read-benchmarks.json (git-ignored)
...
import { DatabaseBenchmark } from './read-benchmarks'
...
`)
  ...
  console.log(`Output: benchmark-results/read-benchmarks.json`)
  console.log('')
  ...
  try {
    ...
    const summary = benchmark.runAll({
      ...
      saveToFile: 'benchmark-results/read-benchmarks.json'
    })
